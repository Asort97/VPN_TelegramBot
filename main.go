...
<a href="https://neuravpn.ru/cabinet/">сайт с личным кабинетом</a>`
...
type DataStore interface {
	...
	ClearVerifiedEmail(userID string) error
...
func sendAccess(info *accessInfo, telegramUserID string, chatID int64, addedDays int, userID int64, cfg *xraySettings, bot *tgbotapi.BotAPI, session *UserSession) error {
	...
	subURL := publicSubscriptionURLForUser(cfg, telegramUserID, info)
...
func main() {
	...
	if webhookSecret != "" || mergedSubSecret != "" {
		go func() {
			...
			if mergedSubSecret != "" {
				mux.HandleFunc("/merged-sub/", handleMergedSubscription)
			}
...
func buildMergedSubscriptionURL(userID string) string {
	...
	return fmt.Sprintf("%s/merged-sub/%s/%s", strings.TrimRight(mergedSubPublicBaseURL, "/"), url.PathEscape(userID), token)
}
...
func handleMergedSubCommand(bot *tgbotapi.BotAPI, msg *tgbotapi.Message, primaryCfg *xraySettings) {
	...
	upstreamURL, subID, _, err := buildSubscriptionURLForUser(primaryCfg, userID)
	if err != nil {
...
func handleIncomingMessage(bot *tgbotapi.BotAPI, msg *tgbotapi.Message, xrCfg *xraySettings) {
	...
	if session.State == stateEditEmail {
		...
		if err != nil || addr.Address == "" || !strings.Contains(addr.Address, "@") {
			...
			_ = updateSessionTextRaw(bot, chatID, session, stateEditEmail, "Неверный e-mail. Отправь почту в формате <code>name@example.com</code>.", "HTML", kbRaw)
			return
		}
		_ = userStore.SetEmail(userID, addr.Address)
		_ = userStore.ClearEmailVerification(userID)
		showEmailMenu(bot, chatID, userID, session, "✅ e-mail для чеков изменён. Теперь его можно подтвердить.")
		return
	}
	...
	if session.State == stateVerifyEmail {
		...
		if strings.TrimSpace(verifyEmail) == "" || strings.TrimSpace(verifyCode) == "" || verifyExpires.IsZero() {
			showEmailMenu(bot, chatID, userID, session, "код не найден. Запроси подтверждение заново.")
			return
		}
		if time.Now().After(verifyExpires) {
			_ = userStore.ClearEmailVerification(userID)
			showEmailMenu(bot, chatID, userID, session, "код истёк. Запроси новый код подтверждения.")
			return
		}
		...
		if inUse, err := userStore.IsVerifiedEmailInUse(verifyEmail, userID); err != nil {
			...
			showEmailMenu(bot, chatID, userID, session, "⚠️ эта почта уже подтверждена другим пользователем.")
			return
		}
		...
		_ = userStore.ClearEmailVerification(userID)
		showEmailMenu(bot, chatID, userID, session, "✅ почта подтверждена. Теперь её можно использовать для входа на сайте.")
		return
	}
...
func buildStatusText(cfg *xraySettings, userID int) (string, error) {
	...
	// Always show one public subscription URL to the user.
	subURL := publicSubscriptionURLForUser(cfg, telegramUser, info)
	linkLine := ""
	if strings.TrimSpace(subURL) != "" {
...
func showEmailMenu(bot *tgbotapi.BotAPI, chatID int64, userIDStr string, session *UserSession, notice string) {
	emailRaw, _ := userStore.GetEmail(userIDStr)
	email := strings.TrimSpace(emailRaw)
	if email == "" {
		email = "не указан"
	}
	verifiedEmailRaw, _ := userStore.GetVerifiedEmail(userIDStr)
	verifiedEmail := strings.TrimSpace(verifiedEmailRaw)
	if verifiedEmail == "" {
		verifiedEmail = "не подтверждена"
	}
	showConfirm := normalizeEmail(emailRaw) != "" && normalizeEmail(emailRaw) != normalizeEmail(verifiedEmailRaw)
	...
	if strings.TrimSpace(notice) != "" {
		text = html.EscapeString(strings.TrimSpace(notice)) + "\n\n" + text
	}
...
func handleEmailMenu(bot *tgbotapi.BotAPI, cq *tgbotapi.CallbackQuery, session *UserSession) {
	chatID := cq.Message.Chat.ID
	userIDStr := strconv.FormatInt(cq.From.ID, 10)
	showEmailMenu(bot, chatID, userIDStr, session, "")
	ackCallback(bot, cq, "")
}
...
func handleConfirmEmail(bot *tgbotapi.BotAPI, cq *tgbotapi.CallbackQuery, session *UserSession) {
	...
	if strings.EqualFold(strings.TrimSpace(verifiedEmail), normalized) {
		showEmailMenu(bot, chatID, userIDStr, session, "✅ почта уже подтверждена.")
		ackCallback(bot, cq, "уже подтверждена")
		return
	}

	if !canProceedKey(userID, "email_verify_send", 1*time.Minute) {
		verifyEmail, _, verifyExpires, err := userStore.GetEmailVerification(userIDStr)
		if err == nil && strings.TrimSpace(verifyEmail) != "" && !verifyExpires.IsZero() && time.Now().Before(verifyExpires) {
			text := fmt.Sprintf("📧 код уже отправлен на <b>%s</b>.\n\nвведите код из письма.", html.EscapeString(email))
			kbRaw := rawInlineKeyboardMarkup{InlineKeyboard: [][]rawInlineKeyboardButton{
				{rawCallbackButton("назад", "email_menu", "", "5264852846527941278")},
			}}
			_ = updateSessionTextRaw(bot, chatID, session, stateVerifyEmail, text, "HTML", kbRaw)
			ackCallback(bot, cq, "код уже отправлен")
			return
		}
...
func handleEditEmail(bot *tgbotapi.BotAPI, cq *tgbotapi.CallbackQuery, session *UserSession) {
	...
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("назад", "email_menu"),
		),
	)
...
func handleReferral(bot *tgbotapi.BotAPI, cq *tgbotapi.CallbackQuery, session *UserSession) {
	...
	shareText := url.QueryEscape("подключай vpn, опробовав его бесплатно 5 дней!")
